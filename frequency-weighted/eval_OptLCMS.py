...
def evaluate_OptLCMS(thresholds,epsilon,cms_epsilon,cms_delta):
...
if __name__ == "__main__":  
    ...
    for i in range(len(params["epsilon"])):
        ...
        threshold = params["min_threshold"][i]
        cms_epsilon = params["cms_epsilon"][i]
        cms_delta = params["cms_delta"][i]
        ub_ratios = []
        memory_results = []
        false_rate_results = []
        error_results = []
        ...
        for _ in range(TRIAL):
            u,ub_m,m, f, e = evaluate_OptLCMS(threshold, ep,cms_epsilon,cms_delta)
            ub_ratios.append(u)
    ...
    df.to_csv('results/OptLCMS_result.csv', index=False)